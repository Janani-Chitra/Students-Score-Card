...
class Student
  ...
  def initialize id, sub1, sub2, sub3, sub4
    ...
    raise ArgumentError if @sub1 > 100 || @sub2 > 100 || @sub3 > 100 || @sub4 > 100 #raise if invalid mark values are provided
...
def read_score_details
  ...
  f.each_line do |line|
    input << line if line != "\n"  #read every record from input file - ignore empty lines
  end
  f.close
  input
end
...
srm.score_card_generator read_score_details